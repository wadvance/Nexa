...
class VoiceCommands {
  ...
  Future<String> execute(
    ...
    if (_isOwnerCommand(cmd)) {
      ...
      return await _handleOwnerCommand(rawCommand, cmd, safeContext, uid, domain: domain); // ignore: use_build_context_synchronously
    }
    ...
    AppLogger.debug('[DEBUG nav fallback] Intentando navegación...');
    if (_any(cmd, ['farmacia', 'tienda', 'restaurante', 'taller', 'super',
      ...
      AppLogger.debug('[DEBUG nav fallback] Match! Abriendo navegación.');
      return _openNavigation(rawCommand);
    }

    // Fallback final: respuesta genérica del cerebro local
    AppLogger.debug('[DEBUG fallback final] localResp="$localResp"');
    await ConversationMemoryService.addAssistant(localResp, topic: 'general');
    return localResp;
  }
  ...
  Future<String> _handleOwnerCommand(
      String raw, String cmd, BuildContext context, String uid, {KnowledgeDomain? domain}) async {
    ...
    return _askGemini(raw, domain: domain);
  }
  ...
  String _openNavigation(String raw) {
    ...
    if (dest.isNotEmpty && dest.length > 2) {
      ...
      launchUrl(Uri.parse('https://waze.com/ul?q=$enc&navigate=yes'), mode: LaunchMode.externalApplication);
    ...
    launchUrl(Uri.parse('https://waze.com/ul'), mode: LaunchMode.externalApplication);
  ...
  Future<String> _askGemini(String question, {KnowledgeDomain? domain}) async {
    try {
      // Si tenemos un dominio detectado (p.ej. apicultura → agronomía),
      // usar el system prompt especializado para enrutar la respuesta correctamente.
      final systemPrompt = (domain != null)
          ? KnowledgeDomainService.systemPromptForDomain(domain)
          : '';
      final resp = await AetherisBrain.getExpertAdvice(question, systemPrompt);
